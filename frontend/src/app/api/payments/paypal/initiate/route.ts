...
import { createTransaction, PayPalMode } from '@/lib/payments/paypal';

// POST /api/payments/paypal/initiate — a customer (or staff, on the customer's
// behalf) pays a ticket's estimate directly to that shop, using the shop's own
// PayPal credentials (configured by the shop owner in Settings).
// NOTE: PayPal does not support PKR — amounts are charged in USD regardless of the
// shop's local currency; that conversion nuance isn't handled here (flagged, not fixed).
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const gwConfig = config?.paypal;
    if (!gwConfig?.enabled) {
      return sendResponse(false, 'This shop has not enabled PayPal payments yet', null, 400);
    }

    const mode: PayPalMode = gwConfig.mode || 'sandbox';
    ...
    const result = await createTransaction(
      {
        clientId: gwConfig.clientIdEnc ? decrypt(gwConfig.clientIdEnc) : '',
        clientSecret: gwConfig.clientSecretEnc ? decrypt(gwConfig.clientSecretEnc) : '',
        mode,
      },
      {
        orderId,
    ...
    if (!result.success) {
      ...
      await payment.save();
      return sendResponse(false, result.error || 'PayPal payment could not be started', null, 502);
    }
