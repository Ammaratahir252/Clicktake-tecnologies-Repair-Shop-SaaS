    ...
    <div className="space-y-6">
      ...
      {data && data.lowStockCount > 0 && (
        <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4">
          ...
          <button onClick={() => handleLowStock(!lowStockOnly)} className="ml-auto text-xs font-bold text-amber-600 underline hover:text-amber-800">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all"
          />
        </div>
        <div className="relative min-w-[160px]">
          ...
          <select
            ...
            className="w-full pl-8 pr-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-purple-500 appearance-none cursor-pointer"
          >
        ...
        <button
          ...
          className={`inline-flex items-center gap-2 text-sm font-bold px-4 py-2.5 rounded-xl border transition-all ${
            lowStockOnly ? "bg-red-600 text-white border-red-600" : "bg-white text-slate-600 border-slate-200 hover:border-red-300 hover:text-red-600"
          }`}
        >
          <TrendingDown size={14} /> Low Stock Only
        </button>
        <button onClick={() => fetchParts(search, category, lowStockOnly)} className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-500 hover:bg-slate-100 transition-all">
          <RefreshCw size={15} />
        </button>
      </div>

      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
        {loading && <div className="flex items-center justify-center py-16 gap-3 text-slate-400"><Loader2 className="animate-spin w-5 h-5" /><span className="text-sm">Loading parts…</span></div>}
        {!loading && error && <div className="flex items-center gap-3 m-6 bg-red-50 border border-red-100 rounded-xl p-4"><AlertTriangle size={16} className="text-red-400" /><p className="text-sm font-semibold text-red-600">{error}</p></div>}

        {!loading && !error && data?.parts.length === 0 && (
          <div className="text-center py-20">
            <Package size={40} className="mx-auto text-slate-200 mb-3" />
            <p className="font-bold text-slate-500">No parts found</p>
            <p className="text-sm text-slate-400 mt-1">{search || category || lowStockOnly ? "Adjust your filters" : "Add your first part to get started"}</p>
          </div>
        )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {data.parts.map(part => {
                  ...
                  return (
                    <tr key={part._id} className={`border-b border-slate-50 hover:bg-slate-50/60 transition-colors ${isOut ? "bg-red-50/60" : isLow ? "bg-amber-50/40" : ""}`}>
                      <td className="px-5 py-3.5 font-bold text-slate-800 text-sm">{part.name}</td>
                      <td className="px-5 py-3.5"><code className="text-xs font-mono font-semibold text-slate-500 bg-slate-100 px-2 py-0.5 rounded">{part.sku}</code></td>
                      <td className="px-5 py-3.5"><span className="text-xs font-bold px-2.5 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-100">{part.category}</span></td>
                      <td className="px-5 py-3.5"><StockBadge qty={part.quantity} limit={part.lowStockLimit} /></td>
                      <td className="px-5 py-3.5 text-sm text-slate-500 font-medium">{part.lowStockLimit}</td>
                      <td className="px-5 py-3.5 text-sm text-slate-600 font-semibold">{formatPKR(part.costPrice)}</td>
                      <td className="px-5 py-3.5 text-sm font-bold text-emerald-700">{formatPKR(part.sellPrice)}</td>
                      <td className="px-5 py-3.5">
                        <a href={`/dashboard/manager/inventory/${part._id}`} id={`mgr-view-${part._id}`} className="inline-flex items-center gap-1 text-xs font-bold text-purple-600 hover:text-purple-800 bg-purple-50 hover:bg-purple-100 border border-purple-100 px-3 py-1.5 rounded-lg transition-all">
            ...
            <div className="px-5 py-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400 font-medium">
              ...
              <div className="flex items-center gap-3">
                {data.outOfStockCount > 0 && <span className="text-red-500 font-bold">🚫 {data.outOfStockCount} out of stock</span>}
                {data.lowStockCount > 0 && <span className="text-amber-600 font-bold">⚠ {data.lowStockCount} low stock</span>}
              </div>
            </div>
          </div>
        )}
