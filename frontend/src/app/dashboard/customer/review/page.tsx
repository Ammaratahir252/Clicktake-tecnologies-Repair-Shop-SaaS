...
interface CategoryRatings { [key: string]: number }
...
const npsLabels: Record<number, { label: string; icon: React.ReactNode }> = {
  1: { label: 'Very Poor',  icon: <Frown className="w-5 h-5" /> },
  2: { label: 'Poor',       icon: <Frown className="w-5 h-5" /> },
  3: { label: 'Fair',       icon: <Meh className="w-5 h-5" />   },
  4: { label: 'Good',       icon: <Smile className="w-5 h-5" /> },
  5: { label: 'Excellent',  icon: <Heart className="w-5 h-5" /> },
}

const ratingBarColors: Record<number, string> = {
  1: 'bg-red-500', 2: 'bg-yellow-500', 3: 'bg-yellow-500', 4: 'bg-blue-500', 5: 'bg-green-500',
}

function StarRow({ value, hovered, onRate, onHover, onLeave, size = 28 }: {
  value: number; hovered: number; onRate: (v: number) => void;
  onHover: (v: number) => void; onLeave: () => void; size?: number;
}) {
    ...
    <div className="flex gap-1" onMouseLeave={onLeave}>
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          ...
          onClick={() => onRate(star)} onMouseEnter={() => onHover(star)}
        ...
        >
          <Star size={size} className={`${active >= star ? 'fill-yellow-400 text-yellow-400' : 'text-muted'} transition-colors`} />
        </button>
      ))}
    </div>
  )
...
function NpsScore({ score }: { score: number }) {
  ...
  return (
    <div className="text-center">
      <div className="w-24 h-24 rounded-full bg-muted border-4 border-primary flex flex-col items-center justify-center mx-auto">
        <div className="text-3xl">{meta.icon}</div>
        <div className="text-base font-bold">{score.toFixed(1)}</div>
      </div>
      <Badge className="mt-2">{meta.label}</Badge>
      <Progress value={(score / 5) * 100} className="mt-2 h-1.5" />
...
function ReviewContent() {
  ...
  useEffect(() => {
    api.get('/api/tickets?status=delivered')
      .then((res) => {
  ...
  const catAvg       = ratedCats.length > 0 ? ratedCats.reduce((a, b) => a + b, 0) / ratedCats.length : 0
  const overallScore = npsScore > 0 ? (catAvg + npsScore) / (ratedCats.length > 0 ? 2 : 1) : catAvg
  ...
  async function handleSubmit() {
    ...
    try {
      await api.post('/api/reviews', {
        ticketId:     ticket?._id,
        technicianId: ticket?.technicianId?._id,
      ...
      <div className="flex items-center justify-center py-12 min-h-[60vh]">
        <Card className="max-w-md w-full text-center">
          <CardContent className="pt-12 pb-8 px-6">
            ...
            <p className="text-muted-foreground mb-6">Your review helps us improve and serve future customers better.</p>
  ...
  if (loadingTicket) {
    return (
      <div className="flex items-center justify-center py-20 text-muted-foreground">
        ...
        <span className="font-medium">Loading your recent repair…</span>
      </div>
    )
  }
      ...
      <div>
        ...
        {ticket ? (
          <p className="text-sm text-muted-foreground">
            Repair <strong className="text-primary">{ticket.ticketNumber}</strong> · {ticket.deviceBrand} {ticket.deviceModel}
          </p>
        ) : (
          <p className="text-sm text-muted-foreground">No recently completed repair found.</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
                  ...
                  <div key={cat.key}>
                    ...
                    <div className="flex items-center gap-3">
                      <StarRow
                        ...
                        onRate={(v) => setRatings((p) => ({ ...p, [cat.key]: v }))}
                        onHover={(v) => setHovered((p) => ({ ...p, [cat.key]: v }))}
                        onLeave={() => setHovered((p) => ({ ...p, [cat.key]: 0 }))}
                      />
                      {val > 0 && (
          ...
          <Card>
            ...
            <CardContent>
              <Label className="font-semibold mb-3 block text-sm">How likely are you to recommend us to a friend?</Label>
              <div className="flex justify-center mb-4">
                <StarRow
                  value={npsScore} hovered={npsHover}
                  onRate={setNpsScore} onHover={setNpsHover}
                  onLeave={() => setNpsHover(0)} size={36}
                />
              </div>
          ...
          <Card>
            ...
            <CardContent>
              ...
              />
              <div className="text-right text-xs text-muted-foreground mt-1">{feedback.length} / 500</div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
            ...
            <CardContent>
              {overallScore > 0 ? (
                ...
                <div className="text-center text-muted-foreground text-sm py-8">
                  ...
                  Start rating to see your overall score
                </div>
              )}
                    ...
                    <div key={cat.key} className="flex items-center gap-2">
                      ...
                      <div className="flex-1">
                        <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                          <div className={val > 0 ? ratingBarColors[val] : 'bg-muted'} style={{ width: `${(val / 5) * 100}%`, height: '100%' }} />
                        </div>
                      </div>
                      <span className="text-xs w-8 text-right text-muted-foreground">{val > 0 ? `${val}/5` : '—'}</span>
                    </div>
                  )
                })}
              </div>
            ...
            <Card>
              <CardContent className="pt-6 space-y-3">
                ...
                <div>
                  ...
                  <div className="text-sm font-medium">{ticket.deviceBrand} {ticket.deviceModel}</div>
                </div>
                <div>
          ...
          <Button className="w-full" disabled={!canSubmit || submitting || !ticket} onClick={handleSubmit}>
            {submitting ? (
              <>
          ...
          <Button variant="ghost" className="w-full" size="sm" asChild>
            <a href="/dashboard/customer">Skip for Now</a>
          </Button>
        </div>
      </div>
    </div>
