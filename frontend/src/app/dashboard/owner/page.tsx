...
function OwnerContent({ user }: { user: any }) {
  ...
  // Home widget preferences (shop-customization)
  ...
  useEffect(() => {
    // Fetch home widget preferences
    api.get("/api/tenant/branding")
      .then((res) => {
        const prefs = res.data?.data?.branding?.homeWidgets;
        if (prefs) {
          setHomeWidgets({
            shopIdCard: prefs.shopIdCard ?? true,
            moduleShortcuts: prefs.moduleShortcuts ?? true,
            teamTable: prefs.teamTable ?? true,
            lowStockBadge: prefs.lowStockBadge ?? true,
          });
        }
      })
      .catch(() => { /* non-critical */ });
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {MODULES.map(({ key, icon: Icon, title, desc, href, color }) => (
              <Link key={key} href={href}
                ...
                {homeWidgets.lowStockBadge && key === "inventory" && lowStockCount > 0 && (
