...
/**
 * app/dashboard/owner/forecast/page.tsx
 * Module 8.3 — Owner view of AI Demand Forecaster
 *
 * Re-exports the same component with owner role guard.
 * RBAC: owner
 */
...
import {
  BarChart2, RefreshCw, AlertTriangle, CheckCircle2,
  TrendingDown, TrendingUp, Minus, Loader2, Package, Lightbulb, Zap,
} from "lucide-react";
...
type ForecastAlert = {
  partName: string; sku: string; currentStock: number;
  urgency: Urgency; recommendedReorderQty: number; reason: string;
};

type ForecastResult = {
  summary: string; alerts: ForecastAlert[]; insights: string[];
  overallHealth: Health; partsAnalysed: number; movementsAnalysed: number; model: string;
};
...
export default function OwnerForecastPage() {
  return (
...
function ForecastContent() {
  ...
  const runForecast = async () => {
    ...
    try {
      ...
      else setError(res.data.message ?? "Forecast failed");
    } catch (e: any) {
      ...
      {!loading && !result && !error && (
        <div className="text-center py-20 bg-card border border-border/60 rounded-2xl">
          ...
          <p className="text-sm text-muted-foreground max-w-xs mx-auto">
            Click "Run AI Forecast" to analyse your inventory with Claude.
          </p>
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className={`border rounded-2xl p-6 ${healthCfg?.bg} ${healthCfg?.bg.replace("bg-", "border-").replace("/10", "/30")}`}>
            ...
            <div className="mt-4 flex items-center gap-4 flex-wrap">
              <span className="text-xs text-muted-foreground bg-background/60 border border-border/40 rounded-full px-3 py-1 font-semibold">{result.partsAnalysed} parts analysed</span>
              <span className="text-xs text-muted-foreground bg-background/60 border border-border/40 rounded-full px-3 py-1 font-semibold">{result.movementsAnalysed} movements (30d)</span>
              <span className="text-xs text-muted-foreground bg-background/60 border border-border/40 rounded-full px-3 py-1 font-semibold flex items-center gap-1"><Zap size={10} /> {result.model}</span>
            </div>
          </div>
          ...
          {result.alerts.length > 0 && (
            <section>
              <h2 className="text-sm font-black text-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                <AlertTriangle size={14} className="text-amber-500" /> Reorder Alerts ({result.alerts.length})
              </h2>
              <div className="space-y-3">{result.alerts.map((a, i) => <AlertCard key={i} alert={a} />)}</div>
            </section>
          )}
          ...
          {result.alerts.length === 0 && (
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-5 flex items-center gap-3">
              ...
              <p className="text-sm text-emerald-700 font-medium">No urgent reorder alerts — all parts are above their thresholds.</p>
            </div>
          )}
          ...
          {result.insights.length > 0 && (
            <section>
              <h2 className="text-sm font-black text-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                <Lightbulb size={14} className="text-primary" /> AI Insights
              </h2>
              <div className="space-y-2">
                {result.insights.map((insight, i) => (
                  <div key={i} className="bg-card border border-border/60 rounded-xl px-4 py-3 flex items-start gap-3">
                    <span className="w-5 h-5 bg-primary/15 text-primary font-black text-xs rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">{i + 1}</span>
