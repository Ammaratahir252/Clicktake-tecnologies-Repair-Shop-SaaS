...
const JOB_STATUSES = [
  ...
  { key: "en_route",   label: "En Route",    color: "bg-blue-100 text-blue-700" },
  { key: "arrived",    label: "Arrived",     color: "bg-amber-100 text-amber-700" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Total Jobs",     value: jobs.length,         color: "text-slate-800" },
          { label: "Active",         value: activeJobs.length,   color: "text-orange-600" },
          { label: "Completed Today",value: completedToday.length,color: "text-emerald-600" },
      ...
      <section className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-slate-800">Today's Jobs</h2>
          </div>
        ...
        {!loading && !error && jobs.length === 0 && (
          <div className="text-center py-12">
            <Package className="mx-auto text-slate-200 w-10 h-10 mb-3" />
            <p className="text-slate-400 font-semibold text-sm">No jobs assigned today.</p>
            <p className="text-slate-300 text-xs mt-1">Check with your manager for assignments.</p>
          </div>
        )}
          ...
          <div className="divide-y divide-slate-50">
            {jobs.map((job, i) => (
              <div
                ...
                className="px-6 py-4 flex items-center justify-between gap-4 flex-wrap hover:bg-slate-50/50 transition-colors"
              >
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0
                    ${job.jobType === "pickup" ? "bg-purple-100" : "bg-emerald-100"}`}>
                    {job.jobType === "pickup"
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      <p className="text-xs text-slate-400">{job.address?.street ?? "Address not set"}</p>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
      ...
      <div className="bg-orange-50 border border-orange-200 rounded-2xl px-5 py-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-bold text-orange-800">Driver Access</p>
