...
import { LogOut, ArrowRight, Wrench, Wallet, Clock } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const handleLogout = async () => {
    setLoggingOut(true);
    try {
      // 1. Hit the server-side logout route — clears the httpOnly cookie reliably
      await fetch('/api/auth/logout', { method: 'POST' });
    } catch (_) {
      // Even if the request fails, continue with client-side cleanup
    } finally {
      // 2. Clean up client-side storage
      localStorage.clear();
      sessionStorage.clear();
      // 3. Navigate — cookie is already gone server-side, middleware won't bounce us back
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <header className="bg-white rounded-3xl p-6 flex flex-col md:flex-row justify-between items-center shadow-sm border border-slate-200/60">
          ...
          <button
            ...
            disabled={loggingOut}
            className="mt-4 md:mt-0 flex items-center gap-2 bg-red-50 text-red-600 px-6 py-2.5 rounded-xl hover:bg-red-100 transition-all font-bold text-sm border border-red-100 shadow-sm shadow-red-50 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <LogOut size={16} />
